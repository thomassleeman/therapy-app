...
function getResultSummary(output: unknown): string | null {
  ...
  if (count === 0) return "No matching sources found";

  const sourceWord = count === 1 ? "source" : "sources";

  if (tier === "high") return `Found ${count} relevant ${sourceWord}`;
  if (tier === "medium")
    return `Found ${count} ${sourceWord} (moderate confidence)`;
  if (tier === "low") return `Found ${count} ${sourceWord} (low confidence)`;
